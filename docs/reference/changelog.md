...
- `capture_dirty_state()` and `reset_dirty_state()` now accept a single `DirtyFieldsMixin` instance in addition to an iterable. Existing iterable callers are unaffected.
...
- **Breaking:** import name renamed from `dirtyfields` to `filthyfields`. Update `from dirtyfields import …` to `from filthyfields import …`. PyPI distribution name unchanged.
- LICENSE rewritten with own copyright + attribution to upstream (Praekelt Foundation original, romgar's fork). Terms (BSD-3-Clause) unchanged.
- `FIELDS_TO_CHECK` + `FIELDS_TO_CHECK_EXCLUDE` mutual-exclusion check now fires at class def time, not first write.
- `_normalize_value` compiled as a Cython `ccall`; `all(generator)` rewritten as for-loops with early return.

### Fixed

- `save(update_fields=…)` no longer wipes dirty state for fields it didn't persist.
- `is_dirty()` now honors `compare_function` (matched `get_dirty_fields()`).
- TRACK_MUTATIONS snapshot no longer skipped on first read of a deferred field.
- `__set__`: equality computed before the dict write so a raise in `_values_equal` doesn't tear `__dict__`.
- `__get__` `_state_mut_snapshot` init uses `setdefault` — closes free-threading race.
- API docs: `save_dirty_fields` doesn't raise on never-saved (it falls back to `save()`); `normalise_function` example replaced with a real transformation.

### Added

- Tests: custom `compare_function` callable, custom `normalise_function` callable, partial-save reset, TRACK_MUTATIONS read-without-track-flag, `arefresh_from_db(from_queryset=…)`, `reset_dirty_state(fields=…)` for FK. `isolated_media_root` fixture so file-field tests don't leak files.
...
- `TRACK_MUTATIONS` opt-in: detects in-place mutations of mutable field values (`dict`, `list`, `set`, `bytearray`).

### Fixed

- `F()` and other ORM expressions are no longer reported as dirty — they're directives the ORM resolves at save time, not value changes.
